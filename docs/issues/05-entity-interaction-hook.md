...
- [ ] The Engine emits an Interaction Engine Event identifying which Entity was interacted with, sourced from the object's dedicated `entityId` property (not its `name` field)
- [ ] The Engine warns (`console.warn`, not a thrown error) when two Entity objects on the same Map share an `entityId`, via a shared `findDuplicates` pure function
